...
def run_tkinter():
    ...
    BLINK_PROBABILITY = 0.2   # Probability to trigger a blink event on each move
    ...
    def choose_direction():
        """Randomly pick a direction (left, right, up, down) 
           as long as it hasn't reached its max moves."""
    ...
    def animate_ball():
        ...
        # Decide if this move will be a blink instead
        # (blink = event="blink", speed=0, no ball movement)
        blink_event = False
        if current_blink_count < MAX_BLINKS and random.random() < BLINK_PROBABILITY:
            blink_event = True
            current_blink_count += 1
        
        if blink_event:
            # Show blink text
            blink_text_item = canvas.create_text(center_x, center_y - 150, 
                                                 text="Blink Twice",
                                                 font=("Helvetica", 32), 
                                                 fill="black")
            # After 1 second, remove text and log the blink event
            def finalize_blink():
                canvas.delete(blink_text_item)
                move_log.append({
                    "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
                    "event": "blink",
                    "speed": 0
                })
                # No movement for blink; just return to center after rest
                canvas.after(REST_DELAY, return_to_center)
            
            canvas.after(1000, finalize_blink)
        
        else:
            # Not a blink, so we do a normal move
